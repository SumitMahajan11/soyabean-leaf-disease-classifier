"""
ADVANCED REGULARIZED TRAINING FOR SOYBEAN DISEASE CLASSIFICATION
Target: Push accuracy beyond 98.14% using advanced regularization and optimization techniques
"""
...
def create_advanced_regularized_transforms(train_phase=True):
...
def create_advanced_regularized_model(num_classes):
    """Create a more advanced EfficientNet with additional regularization"""
    # Use EfficientNet-B5 for even better performance
    model = models.efficientnet_b5(pretrained=True)
    model.classifier[1] = nn.Linear(model.classifier[1].in_features, num_classes)
    
    # Add additional dropout for regularization
...
def focal_loss(inputs, targets, alpha=1, gamma=2, weight=None):
...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        return focal_loss(inputs, targets, self.alpha, self.gamma, self.weight)

def train_advanced_regularized_model(model, train_loader, val_loader, num_epochs=100, model_name="AdvancedRegularizedEfficientNet", save_dir="CNN_trained_models"):
    """Train the model with advanced regularization and optimization techniques"""
    ...
    criterion = FocalLoss(weight=class_weights, alpha=1, gamma=2)
    ...
    optimizer = optim.AdamW([
        {'params': backbone_params, 'lr': 0.00005},  # Lower LR for backbone
        {'params': classifier_params, 'lr': 0.0001}   # Higher LR for classifier
    ], weight_decay=1e-4, betas=(0.9, 0.999))
    
    # Advanced learning rate scheduling
    scheduler = optim.lr_scheduler.OneCycleLR(
        optimizer, 
        max_lr=[0.0001, 0.0002],  # [backbone_lr, classifier_lr]
    ...
    print(f"Starting advanced regularized training for {model_name}...")
    print(f"Using Focal Loss to handle class imbalance")
    print(f"Using differential learning rates (backbone: 5e-5, classifier: 1e-4)")
    print(f"Using OneCycleLR scheduler")
    print(f"Using EfficientNet-B5 with additional dropout")
    ...
    for epoch in range(num_epochs):
        ...
        print(f'Current LR (Classifier): {current_lr_classifier:.2e}')
    ...
    with open(save_path / "training_log.txt", 'w') as f:
        f.write(f"Advanced Regularized Model: {model_name}\n")
        f.write(f"Best Accuracy: {best_acc:.4f}\n")
        f.write(f"Best Epoch: {best_epoch}\n")
        f.write(f"Training completed at: {datetime.now()}\n")
        f.write(f"Used EfficientNet-B5\n")
        f.write(f"Used Focal Loss for class imbalance\n")
        f.write(f"Used differential learning rates\n")
        f.write(f"Used OneCycleLR scheduler\n")
        f.write(f"Used additional dropout regularization\n")
...
def main():
    print("Starting Advanced Regularized Soybean Disease Classification Training")
    print(f"Using device: {device}")
    
    # Create advanced preprocessing transforms
    train_transform = create_advanced_regularized_transforms(train_phase=True)
    val_transform = create_advanced_regularized_transforms(train_phase=False)
    ...
    # Create data loaders
    batch_size = 2 if device.type == 'cuda' else 1  # Reduced batch size for B5 model
    ...
    # Create advanced regularized model
    print("\nCreating Advanced Regularized EfficientNet-B5 model...")
    model = create_advanced_regularized_model(num_classes)
    
    # Train the advanced regularized model
    print(f"\n{'='*60}")
    print(f"Training Advanced Regularized EfficientNet Model")
    print(f"{'='*60}")
    
    trained_model, best_acc = train_advanced_regularized_model(
        model, train_loader, val_loader, 
        num_epochs=100,  # More epochs with better techniques and early stopping
        model_name="AdvancedRegularizedEfficientNet",
    ...
    print(f"\nAdvanced Regularized EfficientNet completed - Val Acc: {best_acc:.4f}, Test Acc: {test_acc:.4f}")
    
    print("\n" + "="*60)
    print("Advanced regularized training completed!")
    print(f"Final Test Accuracy: {test_acc:.4f} ({test_acc*100:.2f}%)")
    print("Model saved to CNN_trained_models/AdvancedRegularizedEfficientNet/")
    print("="*60)
