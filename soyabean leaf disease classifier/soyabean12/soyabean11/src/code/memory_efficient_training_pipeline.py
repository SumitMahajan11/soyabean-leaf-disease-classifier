...
def train_model(model, train_loader, val_loader, num_epochs=40, model_name="model", save_dir="CNN_trained_models"):
    """Train a single model with early stopping and learning rate scheduling"""
...
def main():
    print("Starting Soybean Disease Classification Training Pipeline - Memory Efficient Version")
    print(f"Using device: {device}")
    ...
    # Create data loaders with reduced memory footprint
    batch_size = 4 if device.type == 'cuda' else 2  # Even smaller batch size for memory efficiency
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=1, pin_memory=False, persistent_workers=False)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, num_workers=1, pin_memory=False, persistent_workers=False)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, num_workers=1, pin_memory=False, persistent_workers=False)
    
    # Define models to train
    models_to_train = ["EfficientNet", "ResNet", "MobileNet", "DenseNet"]
    ...
    for model_name in models_to_train:
        ...
        trained_model, best_acc = train_model(
            ...
            num_epochs=20,  # Reduced epochs for memory management (can be increased later)
            model_name=model_name,
