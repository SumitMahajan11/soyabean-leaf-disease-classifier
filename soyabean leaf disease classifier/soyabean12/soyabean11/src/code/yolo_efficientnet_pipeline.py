"""
YOLO + Enhanced EfficientNet-B3 detection and classification pipeline.

- YOLO (Ultralytics) is used ONLY for leaf/plant detection in full images.
- Enhanced EfficientNet-B3 (98.14% accuracy) is used ONLY for disease classification
  on cropped leaf images, via SoybeanDiseaseClassifierEnhanced.

This script assumes:
- You have trained a YOLO model for single-class `leaf` detection and saved weights as
  something like `runs/detect/train/weights/best.pt`.
- The existing CNN models and metrics are stored under `CNN_trained_models/`.
"""
...
class LeafDetectionAndClassificationPipeline:
    ...
    def __init__(
        ...
        """Initialize YOLO detector and Enhanced EfficientNet classifier.

        Args:
            yolo_weights: Path to trained YOLO weights (.pt) for leaf detection.
            model_dir: Directory containing the EnhancedEfficientNet weights.
            yolo_imgsz: Input image size for YOLO (square, e.g. 640).
            yolo_conf: Confidence threshold for YOLO detections.
            disease_confidence_threshold: Confidence threshold for EfficientNet predictions.
        """
        ...
        # Load Enhanced EfficientNet classifier
