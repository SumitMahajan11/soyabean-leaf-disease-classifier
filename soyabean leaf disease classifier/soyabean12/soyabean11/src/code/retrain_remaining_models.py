...
def train_model(model, train_loader, val_loader, num_epochs=50, model_name="model", save_dir="CNN_trained_models"):
    """Train a single model with early stopping and learning rate scheduling - Extended Training"""
    save_path = Path(save_dir) / f"{model_name}_retrained"
    ...
    print(f"Starting extended training for {model_name} with {num_epochs} epochs...")
    ...
    for epoch in range(num_epochs):
        ...
        # Early stopping: if no improvement for 15 epochs, stop training
...
def main():
    print("Starting extended retraining for MobileNet and DenseNet with 50+ epochs")
    print(f"Using device: {device}")
    ...
    # Create data loaders with reasonable batch size for memory management
    batch_size = 8 if device.type == 'cuda' else 4  # Increased batch size for better training stability
    ...
    # Retrain each model with extended epochs
    results = {}
    
    for model_name in models_to_train:
        print(f"\n{'='*60}")
        print(f"RETRAINING {model_name} with 50+ epochs")
        print(f"{'='*60}")
        
        # Create model
        model = create_model(model_name.lower(), num_classes)
        
        # Train model with 50 epochs (with early stopping)
        trained_model, best_acc = train_model(
            model, train_loader, val_loader, 
            num_epochs=50,  # 50 epochs as requested
            model_name=model_name,
        ...
        print(f"{model_name} retraining completed - Val Acc: {best_acc:.4f}, Test Acc: {test_acc:.4f}")
    ...
    results_path = Path("CNN_trained_models") / "retraining_results.json"
    with open(results_path, 'w') as f:
        json.dump(results, f, indent=4)
    
    print("\n" + "="*60)
    print("EXTENDED RETRAINING completed for both models!")
    print("Final Results:")
    ...
    print(f"\nBest performing model after retraining: {best_model} with test accuracy: {results[best_model]['test_accuracy']:.4f}")
