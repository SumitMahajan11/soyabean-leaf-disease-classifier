...
def process_dataset_with_resume(input_dir, output_dir, progress_file="enhancement_progress.json"):
        ...
        if category_dir.is_dir():
            ...
            for img_file in category_dir.iterdir():
                if img_file.is_file() and img_file.suffix.lower() in ['.jpg', '.jpeg', '.png', '.bmp', '.tiff', '.tif']:
                    ...
                    img_str = str(img_file.relative_to(input_path))
                    if progress_data.get(img_str) == "completed":
                        total_skipped += 1
                        continue
                    
                    # Process the image
                    success = conservative_shadow_removal_enhancement(img_file, output_file)
                    ...
                    if success:
                        ...
                        if total_processed % 50 == 0:  # Save every 50 images
                            with open(progress_file_path, 'w') as f:
                                json.dump(progress_data, f)
                        
                        print(f"Processed ({total_processed} total, {total_skipped} skipped): {img_file.name}")
                    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Conservative Shadow Removal and Background Suppression for Full Soybean Disease Dataset with Resume Capability")
    ...
    parser.add_argument("--progress_file", type=str, default="enhancement_progress.json", help="Progress tracking file")
    
    args = parser.parse_args()
    
    start_time = datetime.now()
    print(f"Starting enhancement process at {start_time}")
    
    process_dataset_with_resume(args.input_dir, args.output_dir, args.progress_file)
